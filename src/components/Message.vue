    ...
    <q-item>
        ...
        <q-item-section top>
            <q-item-label caption class="text-justify">
                <div class="row">
                    <div class="col-xs-12"><span class="text-weight-bold text-capitalize"
                            :class="'text-' + $props.modecolor">{{ data.author }}</span></div>
                    <div class="col-xs-12 text-body1 text-blue-grey-8"><span style="overflow:auto; width:99vw;">{{ data.text
                    }}</span></div>
                    <div class="col-xs-12 xs">
                        <div v-for="msglabel, msglabelkey of $store.getters.getMsgLabels" :key="msglabelkey"
                            class="text-grey-8 q-gutter-xs truncate-chip-labels">
                            <div v-if="msglabel.type == 'listselect'">
                                <q-select v-show="!(data.author == 'alert')" outlined
                                    :color="(label != null ? $props.modecolor : '')"
                                    :label-color="(label != null ? $props.modecolor : '')" v-model="label[msglabelkey]"
                                    @input="updateData()" :options="msglabel.values" :label="toUpper(msglabelkey)"
                                    transition-show="jump-up" transition-hide="jump-up" options-cover class="select-block"
                                    multiple>
                                    ...
                                    <template v-slot:selected-item="scope">
                                        <q-chip v-show="scope.opt.length > 0" removable dense
                                            @remove="scope.removeAtIndex(scope.index)" :tabindex="scope.tabindex"
                                            color='grey-3' :text-color="$props.modecolor"
                                            class="q-ma-none truncate-chip-labels">
                                            {{ scope.opt }}
                                        </q-chip>
                                    </template>
                                </q-select>
                            ...
                            <div v-else>
                                ...
                                <q-chip v-for="val in msglabel.values" :key="msglabelkey + val" clickable size="sm" dense
                                    :color="(isSelected(msglabelkey, val) ? $props.modecolor : '')"
                                    :text-color="(isSelected(msglabelkey, val) ? 'white' : 'dark-grey')" :label="val"
                                    :selected="isSelected(msglabelkey, val)" @click="selectLabel(msglabelkey, val)">
        ...
        <q-item-section top side class="gt-xs col-7">
            <div v-for="msglabel, msglabelkey of $store.getters.getMsgLabels" :key="msglabelkey"
                class="text-grey-8 q-gutter-xs select-block truncate-chip-labels">
                <div v-if="msglabel.type == 'listselect'">
                    <q-select v-show="!(data.author == 'alert')" outlined :color="(label != null ? $props.modecolor : '')"
                        :label-color="(label != null ? $props.modecolor : '')" v-model="label[msglabelkey]"
                        @input="updateData()" :options="msglabel.values" :label="toUpper(msglabelkey)"
                        transition-show="jump-up" transition-hide="jump-up" options-cover class="select-block" multiple>
                        <q-tooltip :delay="200" style="width:50px">{{ msglabel.description }}</q-tooltip>
                        <template v-slot:append>
                            <q-icon name="label" :color="(label != null ? $props.modecolor : '')" />
                        </template>
                        <template v-slot:selected-item="scope">
                            <q-chip v-show="scope.opt.length > 0" removable dense @remove="scope.removeAtIndex(scope.index)"
                                :tabindex="scope.tabindex" color='grey-3' :text-color="$props.modecolor"
                                class="q-ma-none truncate-chip-labels">
                                {{ scope.opt }}
                            </q-chip>
                        </template>
